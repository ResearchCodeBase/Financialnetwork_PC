...
from GCN.model.FinaModel1 import GraphGCN
...
def masked_accuracy(out, labels, mask):
    ...
    positive_probabilities_np = probabilities[mask][:, 1].detach().numpy()
    # 计算F1-Score和AUC
...
def train_step(model, data, optimizer, criterion):
    ...
    out = model(data.x.float(), data.edge_index)  # 模型前向传播

    train_loss = masked_loss(out, data.y, data.train_mask, criterion)   #损失
    train_loss.backward()  # 反向传播
    optimizer.step()  # 优化器梯度下降

    train_accuracy, train_F1, train_AUC,train_mcc,train_gmean = masked_accuracy(out, data.y, data.train_mask)
    return train_loss, train_accuracy, train_F1, train_AUC,train_mcc,train_gmean

def test(model, data, mask):
    model.eval()
    with torch.no_grad():
        out = model(data.x.float(), data.edge_index)
    test_accuracy, test_F1, test_AUC,test_mcc,test_gmean = masked_accuracy(out, data.y, mask)


    return test_accuracy, test_F1, test_AUC,test_mcc,test_gmean,out
...
def train_model(model, data, optimizer, criterion,path, epochs):
    ...
    for ep in range(epochs + 1):
        train_loss, train_accuracy, train_F1, train_AUC,train_mcc,train_gmean = train_step(model, data, optimizer, criterion)
        val_accuracy, val_f1, val_auc,val_mcc,val_gmean ,out= test(model, data, data.val_mask)
        ...
        print(
            "Epoch {}/{}, Train_Loss: {:.4f}, Train_Accuracy: {:.4f},train_F1: {:.4f},train_AUC: {:.4f}, train_mcc: {:.4f},train_geman: {:.4f}, Val_Accuracy: {:.4f},Val_F1: {:.4f},Val_AUC: {:.4f},Val_mcc: {:.4f},Val_gmean: {:.4f}"
            .format(ep + 1, epochs, train_loss.item(), train_accuracy, train_F1, train_AUC, train_mcc,train_gmean,val_accuracy, val_f1,
                    val_auc,val_mcc,val_gmean))
        output = "Epoch {}/{}, Train_Loss: {:.4f}, Train_Accuracy: {:.4f},train_F1: {:.4f},train_AUC: {:.4f}, train_mcc: {:.4f},train_geman: {:.4f}, Val_Accuracy: {:.4f},Val_F1: {:.4f},Val_AUC: {:.4f},Val_mcc: {:.4f},Val_gmean: {:.4f}\n".format(
            ep + 1, epochs, train_loss.item(), train_accuracy, train_F1, train_AUC, train_mcc, train_gmean,val_accuracy, val_f1,
            val_auc, val_mcc, val_gmean)
        # 创建完整的文件路径
    ...
    return train_losses, train_accuracies, val_accuracies,train_F1_scores,train_AUC_scores,val_F1_scores,val_AUC_scores,train_mccs,train_gmeans,val_mccs,val_gmeans

def evaluate_model(model, data):
    test_accuracy, test_F1, test_AUC,test_mcc,test_gmean,out = test(model, data, data.test_mask)
    ...
    pred = out.argmax(dim=1)
    print('模型结果',out)
    print('out', pred )  # 使用最大概率的类别作为预测结果)
    print('y',data.y)


    return test_accuracy, test_F1, test_AUC,test_mcc,test_gmean
...
def main():
    ...
    output_directory = os.path.join("results_noweight", year, name, ratio, time_dir)
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
    # 训练模型
    train_losses, train_accuracies, val_accuracies,train_F1_scores,train_AUC_scores,val_F1_scores,val_AUC_scores,train_mccs,train_gmeans,val_mccs,val_gmeans = train_model(model, data, optimizer, criterion,  output_directory,epochs)
    # 测试模型
    test_accuracy, test_F1, test_AUC,test_mcc,test_gmean= evaluate_model(model, data)
    ...
    with open(text_file_path, 'w') as file:
        ...
        file.write(f"Test AUC Score: {test_AUC}\n")
