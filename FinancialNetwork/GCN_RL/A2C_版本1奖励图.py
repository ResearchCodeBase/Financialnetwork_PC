...
def train_a2c(a2c, graph_model, data, edge_index, edge_weight, adjust_func, epochs=500, importance_values=None, max_importance_value=None):
    ...
    for epoch in range(epochs):
        ...
        # 写入 CSV 文件
    with open('test_rewards_and_risks.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['Epoch', 'Average Test Reward', 'Average Test Risk'])
        for i in range(len(test_rewards_history)):
            csvwriter.writerow([(i + 1) , test_rewards_history[i], test_risk_history[i]])
...
def test_policy(a2c, graph_model, data, edge_index, edge_weight, adjust_func, importance_values, max_importance_value, num_episodes=10):
    total_test_reward = 0
    total_test_risk = 0
    risk_changes = []
    ...
    for _ in range(num_episodes):
        ...
        for step in range(data.num_nodes):
            if remaining_budget <= 0:
                break
            selected_node, rescue_ratio, _ = a2c.select_action(state, remaining_budget, edge_index, edge_weight)
            selected_node = selected_node.item() if isinstance(selected_node, torch.Tensor) else selected_node
            rescue_ratio = rescue_ratio.item() if isinstance(rescue_ratio, torch.Tensor) else rescue_ratio
