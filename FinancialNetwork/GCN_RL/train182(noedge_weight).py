...
# 加载数据集
dataset = torch.load('old_data/1130-182/processed/yoochoose_click_binary_1M_sess.dataset')
...
# 了余弦退火（CosineAnnealing）和热重启（WarmRestart）改变学习率
...
for ep in range(epochs+1):
    ...
    print(
        "Epoch {}/{}, Train_Loss: {:.4f}, Train_Accuracy: {:.4f},train_F1: {:.4f},train_AUC: {:.4f}, train_mcc: {:.4f},train_gmean: {:.4f}, Val_Accuracy: {:.4f},Val_F1: {:.4f},Val_AUC: {:.4f},Val_mcc: {:.4f},Val_g-mean: {:.4f}"
        .format(ep + 1, epochs, train_loss.item(), train_accuracy, train_F1, train_AUC, mcc, gmean, val_accuracy,
                val_f1, val_auc, val_mcc, vall_g_mean))
