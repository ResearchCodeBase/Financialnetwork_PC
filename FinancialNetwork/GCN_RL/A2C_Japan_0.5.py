...
def train_a2c(a2c, graph_model, data, edge_index, edge_weight, adjust_func, epochs=500, normalized_importance_values=None):
    ...
    # dir_name = f'Japan_training_results_{current_time}'
    ...
    # 获取当前时间并生成目标目录名
    current_time = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    dir_name1 = f'Japan0.06_training_results_{current_time}'

    # 定义上级目录名
    parent_dir_name = '0.75RL-result'

    # 在当前工作目录下创建上级目录
    os.makedirs(parent_dir_name, exist_ok=True)

    # 在上级目录下创建目标目录
    dir_name = os.path.join(parent_dir_name, dir_name1)
    os.makedirs(dir_name, exist_ok=True)
...
if __name__ == '__main__':
    ...
    dataset = torch.load(f'foreign_dataset/{country}/{year}/train0.6_val0.15_test0.25/processed/BankingNetwork.dataset')
    country_name = "Japan_2022"
    # 加载银行重要性数据
