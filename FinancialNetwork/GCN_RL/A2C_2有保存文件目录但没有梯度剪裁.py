...
# 定义A2C算法类
class A2C:
    def __init__(self, policy_net, value_net, lr=0.001, gamma=0.98, epsilon=0.3, epsilon_decay=0.99, min_epsilon=0.01):
...
def compute_reward(total_risk_before, total_risk_after, node_risk_before, node_risk_after, rescue_ratio,
                   importance_value, max_importance_value, system_weight=0.6, node_weight=0.4, scale_factor=10,
                   min_reward=1,
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            average_test_reward, average_test_risk = test_policy(a2c, graph_model, data, edge_index, edge_weight, adjust_func,
                                              importance_values, max_importance_value,dir_name,test_count)
        ...
        print(f"Epoch {epoch + 1}/{epochs}, Total Reward: {total_reward}，Step：{count}")
...
def test_policy(a2c, graph_model, data, edge_index, edge_weight, adjust_func, importance_values, max_importance_value, dir_name,test_count,num_episodes=10):
    total_test_reward = 0
    total_test_risk = 0
    risk_changes = []
