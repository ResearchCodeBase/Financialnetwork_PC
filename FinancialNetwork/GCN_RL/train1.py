...
def masked_accuracy(out, labels, mask):
    ...
    correct = (pred[mask] == labels[mask]).sum().item()  # 获取正确标记的节点
    accuracy = int(correct) / int(mask.sum())  # 计算正确率
...
def train(year,type,ratio,dataset):
    ...
    output_directory = os.path.join("results", year, name, ratio, time_dir)
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
